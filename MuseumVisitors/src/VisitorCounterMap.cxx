...
void VisitorCounterMap::analyzeLogLine(const string& vistorEntry)
{
	...
	{
		visitorsInMinute[i] > 0 ? visitorsInMinute[i]++ : visitorsInMinute[i] = 1;
	}
}
...
{
	Logger::getLogger()->log(LogLevel::TRACE, __FILE__, __LINE__, "BEGIN:VisitorCounterMap::getTimeSlotWithMaxVistors");
	auto started = chrono::high_resolution_clock::now();

	int maxVisitors = 0;
	int maxVisitorsStartTime = 0;
	int maxVisitorsEndTime = 0;
	for (auto i = visitorsInMinute.begin(); i != visitorsInMinute.end(); i++)
	{
		if (i->second > maxVisitors)
		{
			maxVisitors = i->second;
			maxVisitorsStartTime = i->first;
			maxVisitorsEndTime = i->first;
		}
		if (i->second == maxVisitors)
		{
			maxVisitorsEndTime = i->first;
	...
	stringstream ss;
	ss << "END:VisitorCounterMap::getTimeSlotWithMaxVistors:Time taken in nanoseconds:" << duration.count();
